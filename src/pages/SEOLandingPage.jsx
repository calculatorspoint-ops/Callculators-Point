...
import { getLandingBySlug } from "@/data/seoLandingData.js";
import { getCalcBySlug, CATEGORIES } from "@/data/calculatorConfigs.js";
...
export default function SEOLandingPage() {
  ...
  if (!landing) return <Navigate to="/calculators" replace />;
  ...
  return (
    <>
      <Helmet>
        <title>{landing.title} | CalculatorsPoint</title>
        <meta name="description" content={landing.description} />
        <meta name="keywords" content={landing.keywords.join(", ")} />
        <meta property="og:title" content={landing.title} />
        <meta property="og:description" content={landing.description} />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={`https://calculatorspoint.com/tools/${landing.slug}`} />
        <link rel="canonical" href={`https://calculatorspoint.com/tools/${landing.slug}`} />
        ...
        <script type="application/ld+json">{JSON.stringify({
          ...
          itemListElement: [
            ...
            { "@type": "ListItem", position: 3, name: landing.h1 },
          ],
        })}</script>
        ...
        {/* HowTo Schema */}
        {landing.content?.howItWorks?.length > 0 && (
          <script type="application/ld+json">{JSON.stringify({
            "@context": "https://schema.org",
            "@type": "HowTo",
            name: `How to use the ${landing.h1}`,
            step: landing.content.howItWorks.map((s, i) => ({
              "@type": "HowToStep",
              position: i + 1,
              text: s,
            })),
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          ...
          <nav style={{ display: "flex", justifyContent: "center", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
            <Link to="/" style={{ fontSize: 12, color: "rgba(255,255,255,.6)", textDecoration: "none" }}>Home</Link>
            <ChevronRight size={12} style={{ color: "rgba(255,255,255,.4)" }} />
            <Link to="/calculators" style={{ fontSize: 12, color: "rgba(255,255,255,.6)", textDecoration: "none" }}>Calculators</Link>
            <ChevronRight size={12} style={{ color: "rgba(255,255,255,.4)" }} />
            <Link to={`/calculator/${calc.slug}`} style={{ fontSize: 12, color: "rgba(255,255,255,.6)", textDecoration: "none" }}>{calc.name}</Link>
            <ChevronRight size={12} style={{ color: "rgba(255,255,255,.4)" }} />
